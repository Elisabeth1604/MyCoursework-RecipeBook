<template>
    <AppPage title="Сохраненные рецепты"> <!-- Использую шаблон AppPage(название и контейнер для контента)
                                            для главной, избранного, профиля и моих рецептов-->
        <div v-if="favouriteRecipes.length"
             class="recipes">
            <AppRecipeCard v-for="recipe in favouriteRecipes"
                           :key="recipe.id"
                           :recipe-id="recipe.id"
...
onMounted( async() =>{
    await store.dispatch('recipe/fetchFavourites');
});
</script>
